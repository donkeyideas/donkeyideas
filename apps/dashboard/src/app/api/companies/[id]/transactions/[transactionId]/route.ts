...
// PUT /api/companies/:id/transactions/:transactionId
export async function PUT(
  ...
  try {
    ...
    if (originalTransaction.affectsPL || originalTransaction.affectsBalance || originalTransaction.affectsCashFlow) {
      const originalDate = new Date(originalTransaction.date);
      const originalPeriod = new Date(originalDate.getFullYear(), originalDate.getMonth(), 1);
      await reverseTransactionEffects(
        ...
        originalTransaction.affectsCashFlow
      );
    }
    ...
    // Apply new transaction effects (cash flow first so balance sheet can sync)
    ...
    return NextResponse.json({ transaction: { ...updatedTransaction, amount: updatedTransaction.amount.toNumber() } });
  } catch (error: any) {
...
async function reverseTransactionEffects(
  ...
  // Reverse the effects by subtracting instead of adding
  await updatePLStatement(companyId, period, type, category, -amount);
  await updateBalanceSheet(companyId, period, type, category, -amount, isCashTransaction);
  await updateCashFlow(companyId, period, type, category, -amount);
}

async function updatePLStatement(companyId: string, period: Date, type: string, category: string, amount: number) {
...
async function updateCashFlow(companyId: string, period: Date, type: string, category: string, amount: number) {
  ...
  if (type === 'revenue') {
    ...
    // Liability increases (borrowing) are financing activities
...
async function recalculateSubsequentCashFlows(companyId: string, updatedPeriod: Date) {
  ...
  for (const cashFlow of subsequentCashFlows) {
    // Update beginning cash to match previous period's ending cash
    await prisma.cashFlow.update({
      where: { id: cashFlow.id },
      data: {
