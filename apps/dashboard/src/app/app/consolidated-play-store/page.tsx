...
export default function ConsolidatedPlayStorePage() {
  ...
  const loadData = async () => {
    ...
    try {
      const response = await api.get(`/companies/consolidated/play-store?dateRange=${dateRange}`);
      setData(response.data);
    } catch (err: any) {
      console.error('Failed to load consolidated Play Store data:', err);
    ...
    <div>
      ...
      <div className="mb-8 flex flex-col sm:flex-row justify-between items-start gap-4">
        <div>
          ...
          <p className="text-white/60 [.light_&]:text-slate-600">
            Consolidated Android app performance across all companies
          </p>
        </div>
      ...
      {data?.aggregated ? (
        <>
          ...
          {data.aggregated.vitalsTimeSeries?.length > 0 && (
            <div className="mb-6">
              <GPVitalsChart
                data={data.aggregated.vitalsTimeSeries}
        ...
        <Card>
          <CardContent className="p-8">
            <div className="text-center">
              ...
              <p className="text-white/60 [.light_&]:text-slate-600">
                Connect your Android apps by adding package names in each company&apos;s Business Profile.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <h3 className="text-sm font-medium text-white/60 [.light_&]:text-slate-600 mb-3">
              Companies Without Play Store Connection
            </h3>
