...
export function GPRecommendationsCard({ companyId }: GPRecommendationsCardProps) {
  ...
  const loadRecommendation = async () => {
    ...
    setError(null);
    try {
      const response = await api.get(`/companies/${companyId}/play-store/recommendations`);
  ...
  const regenerate = async () => {
    ...
    setError(null);
    try {
      const response = await api.post(`/companies/${companyId}/play-store/recommendations`);
      setRecommendation(response.data.recommendation || null);
    } catch (err: any) {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle>AI Play Store Insights</CardTitle>
          </div>
          <Button
            variant="secondary"
            onClick={regenerate}
