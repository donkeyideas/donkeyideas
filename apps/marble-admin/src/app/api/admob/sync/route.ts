 ...
 *
 * POST: manual "Sync now" button (cookie-authenticated admin).
 * GET:  Vercel cron daily auto-sync (Bearer CRON_SECRET).
 *
 * AdMob revenue lags ~24h; we re-sync the trailing window each time so
 * yesterday's final numbers replace earlier estimates.
 */

async function runSync(request: NextRequest, isCron: boolean) {
  if (!isCron) {
...
export async function POST(request: NextRequest) {
  try {
    return await runSync(request, isCronRequest(request));
  } catch (error: any) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!isCron) {
      return NextResponse.json({ error: { message: 'Cron auth required for GET' } }, { status: 401 });
    }
    return await runSync(request, true);
  } catch (error: any) {
    console.error('AdMob sync (cron) error:', error);
    return NextResponse.json(
      { error: { message: error?.message ?? 'AdMob sync failed' } },
      { status: 500 },
    );
